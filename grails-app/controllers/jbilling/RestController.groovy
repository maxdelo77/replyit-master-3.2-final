    ...
    def createUser = {
       ...
       // def List userProperties=['userId','userName','password','languageId','mainRoleId','statusId','currencyId','balanceType']
        try{
            ...
            if(xml.user.balanceType.size() == 1)
            newUser.setBalanceType(Integer.parseInt(xml.user.statusId.text()))
            else
           ...
           if (xml.user.creditCard.size() ==1) {
            ...
            cc.setName(xml.user?.creditCard?.name.text());
            if(xml.user.creditCard.number.size() ==1)
            cc.setNumber(xml.user?.creditCard?.number.text());
            if(xml.user.creditCard.expiry.size() ==1)
                cc.setNumber(xml.user?.creditCard?.expiry.text());
