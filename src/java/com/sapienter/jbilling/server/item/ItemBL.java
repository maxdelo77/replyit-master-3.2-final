...
public class ItemBL {
    ...
    public BigDecimal getPriceByCurrency(Date date, ItemDTO item, Integer userId, Integer currencyId)  {
    	return getPriceByCurrency(date, item, userId, currencyId, null);
    }
    
// return the price calculated with/without quantity (it's depends of PriceModel type)
    public BigDecimal getPriceByCurrency(Date date, ItemDTO item, Integer userId, Integer currencyId, BigDecimal quantity )  {
        if (item.getDefaultPrices() != null && !item.getDefaultPrices().isEmpty()) {
            ...
            PriceModelDTO priceModel = item.getPrice(new Date());
             ...
             if (priceModel != null) {
            	 if(priceModel.getType().equals(PriceModelStrategy.GRADUATED)){
            		 priceModel.applyTo(null, quantity, result, fields, usage, false, date);
            	 }else{
    ...
    public BigDecimal getPrice(Integer userId, Integer currencyId, BigDecimal quantity, Integer entityId, OrderDTO order, boolean singlePurchase)
        ...
        BigDecimal price = getPriceByCurrency(order != null ? order.getPricingDate() : null, item, userId, currencyId, quantity);
    ...
    public BigDecimal getPriceByEventDate(Integer userId, Integer currencyId, BigDecimal quantity, Integer entityId, Date eventDate)
        ...
        try {
            ...
            while(myTask != null) {
                price = myTask.getPrice(item.getId(), quantity, userId, currencyId, pricingFields, price, null, false);
                myTask = taskManager.getNextClass();
            }
        } catch (Exception e) {
            throw new SessionInternalError("Item pricing task error", ItemBL.class, e);
