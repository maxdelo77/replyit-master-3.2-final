...
public class MediationRecordDAS extends AbstractDAS<MediationRecordDTO> {
    ...
    private static final String countMediationRecordsByEntityMediationProcessAndStatusHQL =
            "SELECT count(distinct mediationRecord) " +
                    " FROM MediationRecordDTO mediationRecord " +
                    " WHERE mediationRecord.process.configuration.entityId = :entity " +
                    " and mediationRecord.process.id = :mediationProcessId " +
                    " and mediationRecord.recordStatus = :status";
    ...
    public Long countMediationRecordsByEntityIdMediationProcessAndStatus(
            Integer entityId, MediationRecordStatusDTO status, Integer mediationProcessId) {

        Query query = getSession().createQuery(countMediationRecordsByEntityMediationProcessAndStatusHQL);
        query.setParameter("entity", entityId);
        query.setParameter("status", status);
    ...
    private static final String findByMediationProcessAndStatusesHQL =
            " select mediationRecord " +
                    " FROM MediationRecordDTO mediationRecord join fetch mediationRecord.recordStatus recordStatus " +
                    " WHERE mediationRecord.process.id = :processId " +
                    " AND recordStatus.id in (:recordStatuses)";
    ...
    public List<MediationRecordDTO> findByProcessAndStatuses(Integer mediationProcessId,
                                                             List<Integer> statuses) {

        Query query = getSession().createQuery(findByMediationProcessAndStatusesHQL);
        query.setParameter("processId", mediationProcessId);
        query.setParameterList("recordStatuses", statuses);
        return query.list();
    }
}
