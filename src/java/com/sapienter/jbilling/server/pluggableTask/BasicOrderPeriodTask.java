    ...
    protected Date getViewLimit(Integer userId, Date processDate) {
        try {
            ...
            Date viewLimit = userBL.getBillingUntilDate(
            		userBL.getNextInvoiceDate(processDate),
            		processDate);
    ...
    */
    protected Date verifyEndOfMonthDay(OrderDTO order, Date date) throws TaskException {
        ...
        if (order.getOrderPeriod().getUnitId().equals(Constants.PERIOD_UNIT_MONTH)) {
            ...
            // if the order is pro-rated take from main subscription
            ...
            if (pref.getInt() == 1) {
                current.set(Calendar.DAY_OF_MONTH, 
                		order.getUser().getCustomer().getMainSubscription().getNextInvoiceDayOfPeriod());
            }
