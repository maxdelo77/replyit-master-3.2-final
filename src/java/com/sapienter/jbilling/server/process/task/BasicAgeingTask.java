...
public class BasicAgeingTask extends PluggableTask implements IAgeingTask {
    ...
    public void reviewAllUsers(Integer entityId, Set<AgeingEntityStepDTO> steps, Date today, Integer executorId) {
        ...
        try {
            ...
            while (users.next()) {
                ...
                LOG.debug("Reviewing invoices for user %s using a grace period of %s days.", user.getId(), gracePeriod);
    ...
    public AgeingEntityStepDTO ageUser(Set<AgeingEntityStepDTO> steps, UserDTO user, Date today, Integer executorId) {
        LOG.debug("Ageing user %s", user.getId());
        ...
        if (currentStatusId.equals(UserDTOEx.STATUS_ACTIVE)) {
            ...
            if (ageingStep != null) {
                ...
                nextStatus =  getNextAgeingStep(steps, currentStatusId);
                LOG.warn("User %s is in an invalid ageing step. Moving to '%s'", user.getId(), getStatusDescription(nextStatus));
            }
        }
    ...
    public boolean isInvoiceOverdue(InvoiceDTO invoice, UserDTO user, Integer gracePeriod, Date today) {
        ...
        if (calendar.getTime().before(today)) {
            LOG.debug("Invoice is overdue (due date %s + %s days grace, is before today %s)", 
                    invoice.getDueDate(), gracePeriod, today);
            return true;
        }

        LOG.debug("Invoice is NOT overdue (due date %s + %s  days grace is after today %s)", 
                    invoice.getDueDate(), gracePeriod, today);
        return false;
    }
    ...
    public boolean isAgeingRequired(UserDTO user, AgeingEntityStepDTO currentStep, Date today) {
        ...
        if (calendar.getTime().equals(today) || calendar.getTime().before(today)) {
            LOG.debug("User status has expired (last change %s + %s days is before today %s)", 
                    lastStatusChange, currentStep.getDays(), today);
            return true;
        }

        LOG.debug("User does not need to be aged (last change %s + %s  days is after today %s)", 
                    lastStatusChange, currentStep.getDays(), today);
        return false;
    }
    ...
    public void setUserStatus(UserDTO user, UserStatusDTO status, Date today, Integer executorId) {
        ...
        LOG.debug("Setting user %s status to '%s'", user.getId(), getStatusDescription(status));
        ...
        if (status.getId() == UserDTOEx.STATUS_DELETED) {
            LOG.debug("Deleting user %s", user.getId());
        ...
        if (couldLogin && status.getCanLogin() == 0) {
            LOG.debug("User %s cannot log-in to the system. Suspending active orders.", user.getId());
        ...
        if (!couldLogin && status.getCanLogin() == 1) {
            LOG.debug("User %s can now log-in to the system. Activating previously suspended orders.", user.getId());
