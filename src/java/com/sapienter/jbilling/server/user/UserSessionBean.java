...
public class UserSessionBean implements IUserSessionBean, ApplicationContextAware, PartnerSQL {
                    ...
                    if (dto.getHasChanged()) {
                        ...
                        if (userId != null) {
                            ...
                            for (PaymentDTO auth : (Collection<PaymentDTO>) paymentBl.getHome().findPreauth(userId)) {
                                LOG.debug("New credit card for user with pre-auths. %s", dto);
                                paymentBl.set(auth);
                                paymentBl.delete();
                            }
                        }
        ...
        try {
            ...
            while (it.hasNext()) {
                CreditCardBL bl = new CreditCardBL(((CreditCardDTO)
                                                            it.next()).getId());
                bl.delete(executorId);
                it = userBL.getEntity().getCreditCards().iterator();
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new SessionInternalError(e);
    ...
    public String getWelcomeMessage(Integer entityId, Integer languageId,
        ...
        try {
            AgeingBL age = new AgeingBL();
            LOG.debug("Getting welcome message for %s language %s status %s", 
    ...
    public void processPayout(Integer partnerId)
            throws SessionInternalError {
        try {
            LOG.debug("Processing partner %s", partnerId);
