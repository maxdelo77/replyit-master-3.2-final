...
public class WebServicesSessionSpringBean implements IWebServicesSessionBean {
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(WebServicesSessionSpringBean.class));
    ...
    public void removePaymentLink(Integer invoiceId, Integer paymentId) {
        ...
        if(paymentBL.getEntity().getIsRefund()==1) {
            LOG.debug("This payment id %s is a refund so we cannot unlink it from the invoice", paymentId);
    ...
    public Integer createInvoiceFromOrder(Integer orderId, Integer invoiceId) throws SessionInternalError {
        ...
        //Set the pessimistic lock (select for update) for a order DTO to ensure no any concurrent process can update it while creating new invoice
    ...
    public Integer createUser(UserWS newUser)
        ...
        if (forceUniqueEmails(entityId) &&
                new UserDAS().findByEmail(newUser.getContact().getEmail(), getCallerCompanyId()).size() > 0){
            throw new SessionInternalError("User already exists with email " + newUser.getContact().getEmail(),
    ...
    private boolean forceUniqueEmails(Integer entityId){
        ...
        try {
            preferenceBL.set(entityId, Constants.PREFERENCE_FORCE_UNIQUE_EMAILS);
        } catch (EmptyResultDataAccessException e) {
            // default preference will be used
        }
        return 1 == preferenceBL.getInt();
    }
    ...
    public Integer createContactTypeWS(ContactTypeWS contactType) throws SessionInternalError {
        ...
        validateBlankContactType(contactType);
        validateDuplicateContactType(contactType);
        ...
        for (InternationalDescriptionWS description : contactType.getDescriptions()) {
            if( ContactBL.validate(description))
            {
    ...
    private void validateBlankContactType(ContactTypeWS contactType) {
    	
    	boolean allBlanks = true;
    	
    	for (InternationalDescriptionWS description : contactType.getDescriptions()) {
    		
    		if (description.getContent() != null && !description.getContent().trim().isEmpty()) {
    			allBlanks = false;
    			break;
    		}	
    	}
    	...
    	if (allBlanks) {
    		throw new SessionInternalError("The contact type is left blank ",
                    new String[]{"ContactTypeWS,name,validation.error.contactType.blank"});
    	}
    		
    	
    }
    
    private void validateDuplicateContactType(ContactTypeWS contactType) {
    	...
    	for (InternationalDescriptionWS description : contactType.getDescriptions()) {
        	...
        	if (description.getContent() != null && !description.getContent().trim().isEmpty()) {
        		...
        		Collection<InternationalDescriptionDTO> list = 
		        		internationalDescriptionDAS.exists(Constants.TABLE_CONTACT_TYPE, 
		        										   Constants.PSUDO_COLUMN_DESCRIPTION, 
		        										   description.getContent().trim(), 
		        										   description.getLanguageId());
		        
		        if (list != null && !list.isEmpty()){
		        	throw new SessionInternalError("The contact type already exists with name " + description.getContent(),
		                    new String[]{"ContactTypeWS,name,validation.error.contactType.already.exists"});
    ...
    public void updateUserContact(Integer userId, Integer typeId, ContactWS contact) throws SessionInternalError {
        ...
        if(forceUniqueEmails(getCallerCompanyId()) &&
                null != contact.getEmail() && (userBL.isEmailUsedByOthers(contact.getEmail()))){
            throw new SessionInternalError("User already exists with email " + contact.getEmail(),
                    new String[] {"ContactWS,email,validation.error.email.already.exists"});
        }
    ...
    public void updateUser(UserWS user)
        ...
        if (dto.getCustomer() != null &&
                dto.getCustomer().getParent() != null &&
                dto.getCustomer().getParent().getId() == dto.getId()) {
        	throw new SessionInternalError("The parent id cannot be the same as user id for this customer.",
        ...
        if (user.getContact() != null) {
            ...
            if(forceUniqueEmails(entityId) && null != email && (bl.isEmailUsedByOthers(email))){
    ...
    /**
     * Retrieves user by the user's email. This is only valid if Jbilling
     * is configured to force unique emails per user/customers in the company.
     * If unique emails are not forced then an exception is thrown and in such
    ...
    public Integer getUserIdByEmail(String email) throws SessionInternalError {
        if (null == email || 0 == email.trim().length()) {
            throw new SessionInternalError("User email can not be null or empty");
        }
        ...
        if (1 == preferenceBL.getInt()) {
            UserDAS userDas = new UserDAS();
            List<UserDTO> users = userDas.findByEmail(email, entityId);
            if (null == users || 0 == users.size()) {
                return null;
            } else if (1 == users.size()) {
                return users.iterator().next().getId();
            } else {
                throw new SessionInternalError("Multiple users found with the same email.");
            }
        } else {
            throw new SessionInternalError("Not configured to force unique emails per users.");
        }
    }
    ...
    public void updateOrder(OrderWS order)
        ...
        try {
            ...
            orderBL.processLines(dto, languageId, entityId, order.getUserId(), order.getCurrencyId(), order.getPricingFields());
    ...
    public OrderWS updateCurrentOrder(Integer userId, OrderLineWS[] lines,
        ...
        try {
            ...
            if (lines != null) {
                ...
                bl.processLines(bl.getEntity(), languageId, getCallerCompanyId(), userId, currencyId, pricing);
                ...
                if (result.getCurrentOrder() == null) {
                    LOG.debug("Call to updateOrder did not resolve to a current order lines = %s fields= %s"
                            ,Arrays.toString(lines), Arrays.toString(fieldsArray));
                    return null;
                }
    ...
    public void deletePayment(Integer paymentId) throws SessionInternalError {
        ...
        // check if the payment is a refund , if it is do not allow it
        if(paymentBL.getEntity().getIsRefund() == 1) {
            LOG.debug("This payment %s is a refund so we cannot delete it.", paymentId);
...
//        payment.setIsRefund(0);
    ...
    public PaymentAuthorizationDTOEx processPayment(PaymentWS payment, Integer invoiceId) {
        ...
        if (payment == null && invoiceId != null) {
            LOG.debug("Payment is null, requesting Payment for Invoice ID %s", invoiceId);
        ...
        LOG.debug("Payment method after %s", dto.getPaymentMethod().getDescription());
        ...
        if (dto != null && dto.getAuthorization() != null) {
            LOG.debug("PaymentAuthorizationDTO Id = %s", dto.getAuthorization().getId());
    ...
    private PaymentDTOEx doPayInvoice(InvoiceDTO invoice, CreditCardDTO creditCard)
        ...
        if (invoice.getBalance() == null || BigDecimal.ZERO.compareTo(invoice.getBalance()) >= 0) {
            LOG.warn("Can not pay invoice: %s, balance: %s", invoice.getId(), invoice.getBalance());
            return null;
        }
    ...
    private ValidatePurchaseWS doValidatePurchase(Integer userId, Integer[] itemIds, String[] fields) {
        ...
        if (itemIds != null) {
            ...
            for (List<PricingField> pricingFields : fieldsList) {
                try {
                    // Since there is no item, run the mediation process rules to create line/s.
    ...
    public List<RecordCountWS> getNumberOfMediationRecordsByStatuses() {
        return getNumberOfMediationRecordsByStatusesByMediationProcess(null);
    }

    public List<RecordCountWS> getNumberOfMediationRecordsByStatusesByMediationProcess(Integer mediationProcessId) {
        IMediationSessionBean mediationBean = Context.getBean(Context.Name.MEDIATION_SESSION);
        Map<MediationRecordStatusDTO, Long> records = mediationBean.getNumberOfRecordsByStatuses(
    ...
    public List<Integer> updateAllMediationConfigurations(List<MediationConfigurationWS> configurations)
        ...
        for(MediationConfigurationWS configuration : configurations){
                if(configuration.getName().length()>50)
                {
                    throw new SessionInternalError("Name is more than 50 characters",
    ...
    public CurrencyWS[] getCurrencies() {
        ...
        try {
            currencies = currencyBl.getCurrencies(getCallerLanguageId(), getCallerCompanyId());
        } catch (SQLException e) {
    ...
    public void updateCurrencies(CurrencyWS[] currencies) throws SessionInternalError {
        ...
        for (CurrencyWS currency : currencies) {
            ...
            if (!currency.getInUse()) {
                // This is a possible de-activation triggered by un-checking the active flag on UI.
                Long inUseCount = 0l;
                Integer entityId = getCallerCompanyId();
                Integer currencyId = currency.getId();

                //currency in use for users
                inUseCount += userDAS.findUserCountByCurrencyAndEntity(currencyId, entityId);

                //currency in use for partners
                inUseCount += partnerDAS.findPartnerCountByCurrencyAndEntity(currencyId, entityId);

                //currrency in use for orders
                inUseCount += orderDAS.findOrderCountByCurrencyAndEntity(currencyId, entityId);

                //currrency in use for products and plans (plan is a product, so no separate handling required)
                inUseCount += itemDAS.findProductCountByCurrencyAndEntity(currencyId, entityId);

                //currrency in use for planItem
                inUseCount += planItemDAS.findPlanItemCountByCurrencyAndEntity(currencyId, entityId);
        ...
        if (!inActiveInUseCurrencies.isEmpty()) {
            ...
            if (inActiveInUseCurrencies.size() > 1) {
                // there is more than one inactive in use currency, so use plural form of the message
                errorMessages[0] = "CurrencyWS,inUse,currencies.updated.currencies.inactive.yet.in.use," + inUseCurrencies;
            } else {
                errorMessages[0] = "CurrencyWS,inUse,currencies.updated.currency.inactive.yet.in.use," + inUseCurrencies;
            }

            LOG.debug("Currency(s) " + inUseCurrencies + " is in use.");
            throw new CurrencyInUseSessionInternalError("Currency(s) "+inUseCurrencies+" is in use.", errorMessages);
        }
    }
    ...
    public Integer createPlugin(PluggableTaskWS plugin) {
    	Integer pluginId= new PluggableTaskBL().create(getCallerId(), new PluggableTaskDTO(getCallerCompanyId(), plugin));
     ...
     */                                                                                                                                           
    /**                                                                                                                                           
     * This method reschedules an existing scheduled task that got changed.                                                                       
     * If not existing, the new plugin may need to be scheduled only if it is                                                                     
     * an instance of {@link IScheduledTask}                                                                                                      
     */
    public void rescheduleScheduledPlugin(Integer pluginId) {
        ...
        try {
            ...
            if (scheduledTask != null) {
                try {
                    SchedulerBootstrapHelper bootStrapper = Context.getBean("schedulerBootstrapHelper");
    ...
    */
    ...
    private IScheduledTask getScheduledTask(Integer pluginId) {
        try {
            ...
            if (Constants.PLUGGABLE_TASK_SCHEDULED.equals(task.getType().getCategory().getId()) ) {
                PluggableTaskManager<IScheduledTask> manager =
                        new PluggableTaskManager<IScheduledTask>
                                (getCallerCompanyId(), com.sapienter.jbilling.server.util.Constants.PLUGGABLE_TASK_SCHEDULED);
                IScheduledTask scheduledTask= manager.getInstance(task.getType().getClassName(),
