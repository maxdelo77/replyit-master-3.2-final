...
public class WSTest {
    ...
    @Test
    public void test002MultipleUserWithSameUserNameCreation() {
    ...
    public void testUserWithParentIdSameAsOwnUserId() {
        try {
            ...
            UserWS user = createUser(true, null, null, false);
            try {
            ...
            try {
            	// invoke update of user after making parent id and user id as same.
            	api.updateUser(user);
            } catch (SessionInternalError e) {
            	System.out.println("A SessionInternalError occurs because the parent id set is the same as user's own id.");
                assertEquals("One error should ocurr", 1, e.getErrorMessages().length);
            }
...
*/
    ...
    @Test
    public void test003CreateUpdateDeleteUser() {
        ...
        try {
            ...
            retUser.getContact().setEmail(retUser.getUserName() + "@shire.com");
...
*/
    ...
    public void test004CreditCardUpdates() throws Exception {
        ...
        // Diners Club
        card.setNumber("3611111111111985");
        ...
        // Discovery
        card.setNumber("6011111111111985");
    ...
    @Test
    public void test006UserTransitions() {
    ...
    */
    @Test
    public void test007ParentChild() {
        ...
        try {
            ...
            newUser.getContact().setEmail(newUser.getUserName() + "@shire.com");
    ...
    */
    ...
    private static Integer createOrder(Integer userId,
    		Integer itemId)
    ...
    public void test008UserMainSubscription() throws Exception {
    	...
    	// now get the user
    	Integer gandalfUserId = 2;
    	UserWS user = api.getUserWS(gandalfUserId);
    	MainSubscriptionWS existingMainSubscription = user.getMainSubscription();
    	System.out.println("Gandalf's existing main subscription = " + existingMainSubscription);

    	MainSubscriptionWS newMainSubscription = new MainSubscriptionWS(2, 1);
    	user.setMainSubscription(newMainSubscription);
    	System.out.println("Gandalf's new main subscription = " + user.getMainSubscription());
    	...
    	assertEquals("User does not have the correct main subscription",
    			newMainSubscription,
    			api.getUserWS(gandalfUserId).getMainSubscription());
    ...
    @Test
    public void test012CreditLimit() {
    ...
    @Test
    public void test013RulesValidatePurchaseTask() {
        System.out.println("#test013RulesValidatePurchaseTask");
        try {
            /*
                Validate purchase runs using the mediation process when pricing fields are provided.

                If there are no pricing fields, then the validation becomes a simple dynamic
                balance check (Credit limit or pre-paid balance) to determine if the customer has
                the funds in their account necessary to make the purchase.
             */
            ...
            // lemonade order
            Integer orderId = createOrder(userId, 2);

            // try to get another lemonde
            ValidatePurchaseWS result = api.validatePurchase(userId, LEMONADE_ITEM_ID, null);
            assertEquals("validate purchase success 1", Boolean.valueOf(true), result.getSuccess());
            assertEquals("validate purchase authorized 1", Boolean.valueOf(false), result.getAuthorized());
            ...
            assertEquals("validate purchase message 2", "Error: Thrown exception for testing", result.getMessage()[0]);
            ...
            assertEquals("validate purchase message 4", "No more than 20 coffees are allowed.", result.getMessage()[0]);
    ...
    @Test
    public void test014UserBalancePurchaseTaskHierarchical() {
    ...
    @Test
    public void test015ValidateMultiPurchase() {
    ...
    @Test
    public void test018UpdateCurrentOrderNewQuantityEvents() {
    ...
    public void test022GetUserByEmail() throws Exception {
        ...
        try {
            ...
            Integer userId = api.getUserIdByEmail("frodo100@shire.com");
            fail("Shouldn't be able to access user by email");
        } catch(Exception e) {
        }

        PreferenceWS uniqueEmailPref = api.getPreference(Constants.PREFERENCE_FORCE_UNIQUE_EMAILS);
        ...
        try {
            System.out.println("Getting valid user by email");
            Integer userId = api.getUserIdByEmail("frodo100@shire.com");
            assertEquals("Returned user with ID", Integer.valueOf(100), userId);
        } catch(Exception e) {
            fail("Shouldn't be able to access user by email");
        }
    ...
    public void test024CreditCardNumberFormat() throws Exception {
        ...
        try {
            ...
            Asserts.assertContainsError(e, "UserWS,creditCard.number,validation.message.error.invalid.pattern");
        }

        user = api.getUserWS(user.getUserId());
        card = user.getCreditCard();
        System.out.println("Original card " + card.getId() + " number: " + card.getNumber());
        assertEquals("card type Visa", Constants.PAYMENT_METHOD_VISA, card.getType());
